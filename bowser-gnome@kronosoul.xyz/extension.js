...
function _enableURIWatcher() {
    try {
        ...
        Me.fileChangedId = Me.fileMonitor.connect("changed", function(monitor, file, other_file, eventType) {
            ...
            strContents = fileUtils.ByteArray.toString(contents);
...
function getxdgDefaultBrowser() {
    try{
    let output = fileUtils.ByteArray.toString(GLib.spawn_command_line_sync('xdg-settings get default-web-browser')[1]).trim();
...
function setxdgDefaultBrowser(browser='bowser-gnome.desktop') {
    try{
    let success = false;
    let output = fileUtils.ByteArray.toString(GLib.spawn_command_line_sync('xdg-settings set default-web-browser ' + browser)[1]).trim();
